...
import { agricultureSlabs2026 } from "../../data/taxSlabs2026";
import { agricultureSlabs2027 } from "../../data/taxSlabs2027";
import { calculateAgricultureTax } from "../../utils/agricultureTaxCalculator";
...
export default function AgricultureTaxCalculator({ taxYear }) {
  const [amount, setAmount] = useState("");
  const [error, setError] = useState("");

  const slabs = taxYear === "2027" ? agricultureSlabs2027 : agricultureSlabs2026;
  const annualIncome = useMemo(() => parseCurrencyInput(amount), [amount]);
  const result = useMemo(() => calculateAgricultureTax(annualIncome, slabs), [annualIncome, slabs]);
  ...
  return (
    <div className="space-y-8">
      ...
      {annualIncome > 0 && (
        <>
          <CopyShareButtons text={`Agriculture Tax: Rs. ${result.annualTax?.toLocaleString("en-PK")}`} />
